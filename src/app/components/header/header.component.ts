...
export class HeaderComponent implements OnInit {
  ...
  getAllFavorites() {
    ...
    this.favoriteService.getByEmail(email).subscribe(data=>{
      ...
      this.toastr.error('Lỗi truy xuất dữ liệu! header 1', 'Hệ thống');
    })
  }

  getAllCartItem() {
    //cai nay la lay email.... nhung may lay cai token vao nen no truy van sai
    //nãy call t nói rồi, xử lý cái getUser là trả về cái
    let email = this.sessionService.getUser();
    if(email==null) {
      return;
    }
    this.cartService.getCart(email).subscribe(data=>{
      ...
      this.cartService.getAllDetail(this.cart.cartId).subscribe(data=>{
        ...
        this.toastr.error('Lỗi server header 2', 'Hệ thống');
      })
    }, error =>{
      this.toastr.error('Lỗi server header 3', 'Hệ thống');
    })
  }
